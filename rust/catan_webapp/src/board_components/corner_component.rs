...
impl Component for CornerComponent {
    type Message = CornerMsg;
    type Properties = CornerProps;
    ...
    fn update(&mut self, msg: Self::Message) -> ShouldRender {
        match msg {
            CornerMsg::Click => {
                debug!("Clicked on corner {}", self.props.coords);
                false
            }
        }
    }
    ...
    fn view(&self) -> Html {
        let corner_transform = match self.props.coords.dir {
...
impl CornerComponent {
    fn view_city(&self) -> Html {
        let style = format!("fill:#{:06X};stroke:none;", self.props.color);
